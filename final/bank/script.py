...
train_raw = pd.read_csv('train.csv/train.csv', parse_dates=['timestamp'])
test_raw = pd.read_csv('test.csv/test.csv', parse_dates=['timestamp'])
...
X_test = test_df.values
#Init Model
xgb = XGBRegressor(learning_rate=0.1, max_depth=8, subsample=0.7, colsample_bytree=0.7)
#Train Model
print ("training...")
model = xgb.fit(X_train, Y_train)
#Make Predictions
